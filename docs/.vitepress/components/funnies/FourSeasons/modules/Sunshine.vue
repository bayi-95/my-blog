<template>
    <!-- 可拖动的太阳 -->
    <div ref="sun" id="sun" :style="style">
        <span></span>
        <span></span>
        <span></span>
        <span></span>
        ...
        <div class="main-wrapper">
            ...
            <div class="clouds-wind-wrapper">
                <div class="clouds-wind-wrap">
                    ...
                    <div class="clouds-wind-section fourth">
                        <div class="cloud">
                            <span></span>
                            <span></span>
                            <span></span>
...
onMounted(() => {
    ...
    import('../utils/shine').then(({ Shine }) => {
        ...
        drawShadow(shadowTitle, initialValue)
        drawShadow(shadowContent, initialValue)
    })
})

// 太阳拖动
const sun = ref<HTMLElement | null>(null)
const { style } = useDraggable(sun, {
    initialValue,
    preventDefault: true,
    stopPropagation: true,
    onEnd: (data) => {
        drawShadow(shadowTitle, data)
 ...
 * @param position 位置
 */
function drawShadow(target: Target, position: Position) {
    const { x, y } = position
    if (target && target.light) {
        target.light.position.x = x
        target.light.position.y = y
        target.draw()
    }
}
</script>
