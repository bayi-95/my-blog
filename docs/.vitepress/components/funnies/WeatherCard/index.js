...
function onResize() {
    ...
    sizes.container.width = document.body.scrollWidth
    sizes.container.height = document.body.scrollHeight
...
function makeRain() {
    ...
    var linerRainHolderMap = {
        1: innerRainHolder1,
        2: innerRainHolder2,
        3: innerRainHolder3
    }

    var line = linerRainHolderMap[3 - Math.floor(lineWidth)].path('M0,0 0,' + lineLength).attr({
        fill: 'none',
...
function makeSnow() {
    ...
    TweenMax.fromTo(
        ...
        3 + Math.random() * 5,
        { x: x, y: y },
        { y: endY, onComplete: onSnowEnd, onCompleteParams: [newSnow], ease: Power0.easeIn }
    )
...
function updateSummaryText() {
    summary.innerHTML = currentWeather.name
